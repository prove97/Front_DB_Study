...
/*
    ...
    사용자가 입력한 사번의 사번, 사원명, 직급코드, 급여, 부서명 조회 후 각 변수에 담아 출력
*/
...
BEGIN
    ...
    IF SAL >= 4000000 THEN LEVEL := '고급';
    ELSIF SAL >= 3000000 THEN LEVEL := '중급';
    ELSIF SAL >= 2000000 THEN LEVEL := '초급';
    ELSE LEVEL := '인턴';
    END IF;
    
    DBMS_OUTPUT.PUT_LINE('해당 사원의 급여등급은 ' || LEVEL || '입니다');    
END;
/
--------------------------------------------------------------------------------
-- 4) CASE 비교대상자 WHEN 동등비교값1 THEN 결과값 WHEN 비교값2 THEN 결과값 ... ELSE 결과값 END;
DECLARE
...
/*
    ...
    EXCEPTION
        ...
        *시스템 예외(오라클에서 미리 정의해둔 예외)
        ...
        - ZERO_DIVIDE : 0으로 나눌때
        - DUP_VAR_ON_INDEX : UNIQUE 제약조건에 위배되었을 경우
          ...
