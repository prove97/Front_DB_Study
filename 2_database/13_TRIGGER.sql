/*
    ...
    입출고에 대한 데이터가 기록(INSERT)될때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야한다.
    ...
    *트리거 종류
    ...
    - SQL문에 의해 영향을 받는 각 행에 따른 분류
     ...
     > 행트리거 : 해당 SQL문 실행할 때마다 매번 트리거 실행
                ...
                > :OLD -BEFORE UPDATE(수정전자료), BEFORE DELETE(삭제전자료)
    ...
    *트리거 생성 구문
    ...
    (DECLARE 변수선언)
    BEGIN
        실행내용(해당 위에 지정된 이벤트 발생시 묵시적(자동)으로 실행할 구문)
    [EXCEPTION 예외처리]
    END;
    /
*/
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, DEPT_CODE, JOB_CODE, HIRE_DATE)
VALUES(501, '김유신', '111211-1111111', 'D7', 'J7', SYSDATE);
...
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰', '샤오미', 400000, 20);
...
-- 2. 상품 입출고 상세이력 테이블생성(TB_PRODETAIL)
...
--TB_PRODUCT테이블에 매번 자동으로 재고 수량 UPDATE 되게끔 트리거 작성
...
/*
    ...
    - 상품이 출고된 경우 -> 해당 상품을 찾아서 재고수량 감수 UPDATE
    UPDATE TB_PRODUCT
