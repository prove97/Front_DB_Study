/*
    ...
    정수 값을 순차적으로 일정 값씩 증가시키면서 생성해줌

    EX) 회원번호, 사원번호, 게시글번호, ...   -> 보통 기본키(PRIMARY KEY)에 사용

    [표현식]
    CREATE SEQUENCE 시퀀스명
    [START WITH 시작숫자] --> 처음 발생시킬 시작값 지정[기본값 : 1]
    [INCREMENT BY 숫자] --> 몇씩 증가시킬지 [기본값 : 1]
...
--[참고] 현재 계정이 소유하고 있는 시퀀스들의 구조를 보고자할 때
...
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; --오류
--> NEXTVAL를 한번도 수행하지 않았기 때문에 CURRVAL를 실행할 수 없음
...
--315가 나와야하지만 MAXVALUE를 310으로 설정했기 때문에 오류발생
...
/*
    ...
    [INCREMENT BY 숫자] --> 몇씩 증가시킬지 [기본값 : 1]
    [MAXVALUE 숫자] --> 최대값 지정[기본값은 매우큰수]
    [MINVALUE 숫자] --> 최소값 지정[기본값은 1] --> 순환할때 사용
    [CYCLE | NOCYCLE] --> 값의 순환여부 지정[기본값 NOCYCLE]
    [NOCACHE | CACHE 바이트크기] --> 캐시메모리 할당(기본값 CACHE 20)
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
    VALUES (SEQ_EID.NEXTVAL, '김말똥', '111111-1222222', 'J6', SYSDATE);
