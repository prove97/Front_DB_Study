...
SELECT EMP_NAME, SALARY*12 AS "연봉", 
                 12*(SALARY*(1+NVL(BONUS, 0))) AS "총수령액(보너스포함)",
                 12*((SALARY*(1+NVL(BONUS, 0))) - (SALARY*0.03)) AS "실수령액"
FROM EMPLOYEE;                 
...
SELECT EMP_NAME, SALARY, 12*((SALARY*(1+NVL(BONUS, 0))) - (SALARY*0.03)) AS "실수령액", HIRE_DATE
FROM EMPLOYEE
WHERE 12*((SALARY*(1+NVL(BONUS, 0))) - (SALARY*0.03)) >= 50000000;
...
--SELECT EMP_NAME, EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 6))) AS 생년
...
SELECT EMP_NAME, DEPT_CODE, 
       ...
       EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "만나이"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6), 
                 SUBSTR(EMP_NO, 8, 7), 
                 TO_NUMBER(SUBSTR(EMP_NO, 1, 6))+TO_NUMBER(SUBSTR(EMP_NO, 8, 7))
FROM EMPLOYEE
WHERE EMP_ID = '201';
...
SELECT DECODE((TO_CHAR( EXTRACT(YEAR FROM HIRE_DATE)), '2001', 1)) AS "2001년"
