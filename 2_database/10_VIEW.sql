...
GRANT CREATE VIEW TO KH; --VIEW 생성 권한 생성(관리자(SYS AS SYSDBA) 계정으로 실행)
...
CREATE OR REPLACE VIEW VW_EMPLOYEE --CREATE OR REPLACE VIEW 뷰명 : 덮어쓰기
AS(SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
   FROM EMPLOYEE
   JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
   JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
/*
    *뷰 컬럼에 별칭부여
    서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어 있을 경우 반드시 별칭을 지정해야한다.
*/
...
/*
    *DML 명령어로 조작이 불가능한 경우가 많다.
    ...
    2) 뷰에 정의되어있지 않은 컬럼 중 베이스테이블 상에 NOT NULL 제약조건이 지정되어있는 경우
    3) 산술연산식 또는 함수식으로 정의가 되어있는 경우
    4) 그룹함수나 GROUP BY절이 포함된 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 이요해서 여러 테이블을 연결시켜놓은 경우
    
    대부분 뷰는 조회를 목적으로 생성
    -> 뷰를 통한 DML은 되도록 안쓰는걸 지향!
*/
/*
    VIEW 옵션
    ...
    1) OR REPLACE : 기존에 동일한 뷰가 있을 경우 갱신하고, 그렇지 않다면 새로 생성
    2) FORCE | NOFORCE
        > FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 생성되도록 해라
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 뷰가 생성되게 하는 옵션(기본값)
    3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능하도록 함
    4) WITH READ ONLY : VIEW에 대해서 조회만 가능하도록 설정
*/
...
--200번 사원 급여를 200만원으로변경(SALARY >= 3000000 조건에 맞지 않는 변경)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
--200번 사원 급여를 200만원으로변경(SALARY >= 3000000 조건에 맞지 않는 변경)이 되지 않음
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
